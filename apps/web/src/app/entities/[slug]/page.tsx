    ...
    <div className="mx-auto grid w-full max-w-7xl gap-6 px-4 py-5 sm:px-6 sm:py-6 lg:grid-cols-[220px_minmax(0,1fr)_280px] lg:px-8 lg:py-8">
      <aside className="hidden lg:block">
        <Card className="sticky top-28 rounded-[1.75rem] p-4">
          ...
          <nav className="mt-3 space-y-2 text-sm text-[var(--text-secondary)]">
            {["overview", "flow", "risks", "notes", "reports"].map((id, index) => {
              const label = ["Overview", "How it normally works", "Risks and red flags", "Important notes", "Related reports"][index];
              return <a key={id} href={`#${id}`} className="block rounded-xl px-2 py-1 transition hover:bg-[var(--bg-surface-muted)] hover:text-[var(--text-primary)]">{label}</a>;
            })}
          </nav>
        </Card>
      </aside>
      <div className="space-y-5">
        <div className="flex flex-wrap gap-2 lg:hidden">
          {["overview", "flow", "risks", "reports"].map((id, index) => {
            const label = ["Overview", "Normal flow", "Risks", "Reports"][index];
            return (
              <a key={id} href={`#${id}`} className="rounded-full bg-[var(--bg-surface)] px-3 py-1.5 text-sm text-[var(--text-secondary)] shadow-[var(--button-secondary-shadow)]">
                {label}
              </a>
            );
          })}
      ...
      <aside className="hidden lg:block">
        <div className="sticky top-28 space-y-4">
          ...
          <Card className="rounded-[1.75rem] p-4">
            ...
            <p className="mt-3 text-sm text-[var(--text-secondary)]">Use this page for normal context first, then compare the attached public reports to see where experiences diverge.</p>
          </Card>
        </div>
      </aside>
    </div>
