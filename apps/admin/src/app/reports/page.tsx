...
import { Card, Input, ModerationBadge, PageHeader, Select, SeverityBadge, VerificationBadge, formatDate, formatEnum } from "@todo/ui";
      ...
      <Card className="overflow-hidden">
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-[var(--border-default)] text-sm">
            ...
            <tbody className="divide-y divide-[var(--border-default)]">{visible.map((report) => <tr key={report.id} className="hover:bg-[var(--bg-surface-muted)]"><td className="px-4 py-3"><Link href={`/reports/${report.id}`} className="font-medium hover:underline">{report.title}</Link><p className="mt-1 max-w-md text-xs text-[var(--text-secondary)]">{report.narrative}</p></td><td className="px-4 py-3 text-[var(--text-secondary)]">{report.entity?.title}</td><td className="px-4 py-3 text-[var(--text-secondary)]">{formatEnum(report.reportType)}</td><td className="px-4 py-3"><SeverityBadge value={report.severityLevel} /></td><td className="px-4 py-3"><VerificationBadge value={report.verificationState} /></td><td className="px-4 py-3"><ModerationBadge value={report.moderationState} /></td><td className="px-4 py-3 text-[var(--text-secondary)]">{formatDate(report.reportedAt)}</td></tr>)}</tbody>
          </table>
        </div>
      </Card>
    </div>
