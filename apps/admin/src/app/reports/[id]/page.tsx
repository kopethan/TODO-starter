...
import { Card, ModerationBadge, PageHeader, SeverityBadge, VerificationBadge, formatDate, formatEnum } from "@todo/ui";
      ...
      <div>
        ...
        <div className="space-y-5">
          <Card className="p-5">
            ...
            <dl className="mt-4 grid gap-4 md:grid-cols-2 text-sm"><div><dt className="text-[var(--text-muted)]">Entity</dt><dd className="mt-1">{report.data.entity ? <Link className="font-medium hover:underline" href={`/entities/${report.data.entity.id}`}>{report.data.entity.title}</Link> : '—'}</dd></div><div><dt className="text-[var(--text-muted)]">Report type</dt><dd className="mt-1">{formatEnum(report.data.reportType)}</dd></div><div><dt className="text-[var(--text-muted)]">Reported</dt><dd className="mt-1">{formatDate(report.data.reportedAt)}</dd></div><div><dt className="text-[var(--text-muted)]">Happened at</dt><dd className="mt-1">{formatDate(report.data.happenedAt)}</dd></div></dl>
          </Card>
